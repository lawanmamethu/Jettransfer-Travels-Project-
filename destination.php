...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {  
    ...
    if ($action === 'add') {
        ...
        $is_active  = 1; // Set new destination as active by default

        // SQL INSERT query to add new destination
    ...
    if ($action === 'update' && isset($_POST['id'])) {
        ...
        $image_path = $conn->real_escape_string($_POST['image_path']);
        $detail_page= $conn->real_escape_string($_POST['detail_page']);
        $is_active  = isset($_POST['is_active']) ? 1 : 0; // Checkbox returns 1 if checked

        // SQL UPDATE query to modify existing destination
    ...
    if ($action === 'delete' && isset($_POST['id'])) {
        $id = (int)$_POST['id']; // Convert to integer for security
        
        // SQL DELETE query to remove destination
        $conn->query("DELETE FROM destinations WHERE id=$id");
    }

    // After any CRUD operation, reload page to see changes
...
<head>
    ...
    <style>
        :root {
            ...
            --primary-dark: #065A7A;      /* Darker shade for hover */
            --secondary: #F59E0B;         /* Accent/warning color */
            ...
            --shadow-lg: 0 10px 40px rgba(0,0,0,0.15);   /* Large shadow */
        }
        ...
        * {
        ...
        .header.scrolled {
            box-shadow: var(--shadow-md);  /* Enhanced shadow on scroll */
        }
        ...
        .hero::before {
            ...
            position: absolute;
            inset: 0;
            background: linear-gradient(160deg, rgba(6,90,122,0.72) 0%, rgba(10,126,164,0.45) 40%, rgba(0,0,0,0.55) 100%);
            z-index: 1;
        }
        ...
        @keyframes emojiDrift {
            ...
            40% {
                transform: translateY(-30vh) rotate(10deg) scale(1.1) translateX(30px);
                opacity: 0.45;
            }
            75% {
                transform: translateY(-65vh) rotate(-8deg) scale(0.95) translateX(-20px);
                opacity: 0.3;
            }
            95% {
                opacity: 0;
            ...
            100% {
                transform: translateY(-90vh) rotate(20deg) scale(0.7) translateX(10px);
                opacity: 0;
            }
        }
        ...
        .section-tag {
            ...
            background: linear-gradient(135deg, rgba(10,126,164,0.1), rgba(16,185,129,0.1));
        ...
        .dest-search-inner button:hover {
            background: var(--primary-dark);
        }
        ...
        .filter-btn.active {
            ...
            color: white;
            transform: translateY(-2px);
            box-shadow: 0 6px 16px rgba(10,126,164,0.3);
        }
        ...
        /* ==============================================
           DESTINATION CARDS (Fixed - Buttons aligned at bottom)
           ============================================== */
        .card-grid {
            max-width: 1200px;
            margin: auto;
        ...
        .why::before {
            width: 500px;
            height: 500px;
            background: radial-gradient(circle, rgba(10,126,164,0.4), transparent);
            top: -150px;
            left: -100px;
        ...
        /* Emoji icons before each list item */
        .why li::before {
            content: "";
            display: block;
            width: 56px;
            ...
            border-radius: 16px;
            font-size: 22px;
            box-shadow: 0 6px 16px rgba(10,126,164,0.35);
        }
        ...
        .why li:hover {
            ...
            box-shadow: 0 20px 45px rgba(10,126,164,0.18);
        }
...
<section class="section" id="destinations">
    ...
    <div class="card-grid" id="cardGrid">
        ...
        <?php 
        // Array mapping destination names to their nearby attractions
        $nearbyAttractions = [
        ...
        <!-- Loop through each destination and display card -->
        <?php foreach($destinations as $i => $d): 
            // Get nearby attractions or use default values
...
window.addEventListener('scroll', () => {
    ...
    // Update reading progress bar width
...
const obs=new IntersectionObserver(entries=>{
    entries.forEach(e=>{
        if(e.isIntersecting){
            const d=e.target.style.getPropertyValue('--i')||0;
            setTimeout(()=>e.target.classList.add('visible'),d*80);
        }
    });
...
// ==============================================
// EMOJI RAIN BACKGROUND EFFECT
// ==============================================
const emojis=['✈️','🌴','🏖️','🗺️','🧳','🌊','🏔️','🌅','🦁','🐘','🦋','🌺','🍃','🏝️','⛵','🌍','🎒','📸','🦜','🌸','🐬','🦚','🌿','🏄','🌈','🎑','🪸','🦩','🐚','🌻'];

function spawnEmoji(c){
    const el=document.createElement('span');
    el.className='emoji-float';
    el.textContent=emojis[Math.floor(Math.random()*emojis.length)];
    el.style.fontSize=(Math.random()*1.6+1.4).toFixed(2)+'rem';
    ...
    setTimeout(()=>el.remove(),(parseFloat(dur)+parseFloat(delay))*1000+500);
}

function startEmojiRain(id){
    const c=document.getElementById(id);
    if(!c)return;
    // Initial burst of emojis
    for(let i=0;i<18;i++) setTimeout(()=>spawnEmoji(c),i*400);
    // Continuous emoji generation
    setInterval(()=>spawnEmoji(c),900);
}

// Start emoji rain effects
