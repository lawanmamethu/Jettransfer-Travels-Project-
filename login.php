...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$email || !$pass) {
        ...
        $r = $conn->query("SELECT * FROM users WHERE email='$e' AND is_active=1 LIMIT 1");
...
<title>Login – Jettransfer Travels</title>
...
.logo-img{width:42px;height:42px;border-radius:12px;object-fit:cover;border:1px solid rgba(2,6,23,.1);background:#fff}
...
.search-box input{border:none;outline:none;padding:.55rem 1rem;font-family:'Manrope',sans-serif;font-size:.88rem;color:var(--text-dark);width:100%;background:transparent}
.search-submit{background:var(--primary);border:none;padding:.55rem 1rem;color:white;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0}
...
.btn-nav-register{padding:.5rem 1.2rem;border-radius:50px;text-decoration:none;font-weight:600;font-size:.88rem;background:var(--primary);color:white;border:2px solid var(--primary);transition:all .3s;white-space:nowrap;box-shadow:0 4px 12px rgba(10,126,164,.3)}
.btn-nav-register:hover{background:var(--primary-dark);border-color:var(--primary-dark);transform:translateY(-2px)}
/* Mobile */
.mobile-toggle{display:none;flex-direction:column;gap:5px;cursor:pointer;padding:8px}
.mobile-toggle span{width:25px;height:3px;background:var(--text-dark);border-radius:2px;transition:all .3s}
...
/* ─── Login card ─── */
.login-section{max-width:500px;margin:3rem auto 5rem;padding:0 2rem}
.login-card{background:white;border-radius:30px;box-shadow:var(--shadow-lg);padding:2.5rem;animation:fadeInUp .8s ease .3s both}
.login-header{text-align:center;margin-bottom:2rem}
.login-header h2{font-family:'Sora',sans-serif;font-size:2rem;font-weight:700;color:var(--primary);margin-bottom:.5rem}
...
.alert-success{background:#D1FAE5;color:#065F46;border:1px solid #A7F3D0;padding:.8rem 1.1rem;border-radius:12px;margin-bottom:1.4rem;font-size:.88rem;font-weight:600}
...
.remember-label{display:flex;align-items:center;gap:.5rem;font-size:.9rem;color:var(--text-light);cursor:pointer}
.forgot-link{color:var(--primary);text-decoration:none;font-size:.9rem;font-weight:600}
...
.btn-submit:hover{background:var(--primary-dark);transform:translateY(-2px);box-shadow:0 6px 18px rgba(10,126,164,.4)}
.register-prompt{text-align:center;font-size:.95rem;color:var(--text-light)}
.register-prompt a{color:var(--primary);text-decoration:none;font-weight:600}
...
<header class="header" id="header">
  <nav class="nav-container">
    ...
    <div class="nav-actions">
      ...
      <!-- Login page: always show Login/Register (user is not logged in) -->
...
<section class="page-header">
  ...
  <p class="page-description">Sign in to your account to manage your bookings and preferences</p>
</section>
...
<section class="login-section">
  <div class="login-card">
    ...
    <form method="POST">
      ...
      <div class="register-prompt">Don't have an account? <a href="register.php">Register here</a></div>
    </form>
  </div>
</section>
...
searchToggle.addEventListener('click', e => { e.stopPropagation(); searchBox.classList.toggle('open'); if(searchBox.classList.contains('open')) setTimeout(()=>document.getElementById('searchInput').focus(),300); });
